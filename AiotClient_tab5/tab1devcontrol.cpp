    ...
    ui(new Ui::Tab1DevControl)
{
    ...
    lcdData = 0;
    pQButonGroup = new QButtonGroup(this);
    ...
    for (int i = 0; i < 8; i++) {
        pQButonGroup->addButton(pQCheckBoxArray[i], i+1);
    }
    pQButonGroup->setExclusive(false);
    connect(pQButonGroup, SIGNAL(buttonClicked(int)), this, SLOT(keyCheckBoxSlotMouse(int)));
